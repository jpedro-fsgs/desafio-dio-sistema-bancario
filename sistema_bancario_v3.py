...
class Conta:
    ...
    def __str__(self):
        return (
            f"Titular da conta: {self.cliente}\n"
...
class ContaCorrente(Conta):
    ...
    def __str__(self):
        return f"Agência: {self.agencia:04} " + f"C/C: {self.numero}"
...
class Historico:
    ...
    def adicionar_transacao(self, transacao):
        ...
        self.transacoes.append({"tipo": tipo, "horario": datetime.now(), "valor": transacao.valor})
    ...
    def extrato(self, mostrar_saque=True, mostrar_deposito=True, hoje=False):
        ...
        for transacao in self.transacoes:
            if hoje and transacao["horario"].date() != datetime.now().date():
                continue
            if (transacao["tipo"] == "Saque" and mostrar_saque) or (
                transacao["tipo"] == "Depósito" and mostrar_deposito
            ):
                output += (
                    f'{transacao["tipo"]} no valor de R$ {transacao["valor"]:.2f} '
                    + f'às {transacao["horario"].strftime('%H:%M do dia %d de %B de %Y')}\n\n'
                )

        if not output:
            return "Histórico de Transações Vazio\n"
...
class Banco:
    ...
    def carregar_clientes(self):
        try:
            with open(Path(__file__).parent / "dados_usuarios copy.json", "r") as dados_usuario_arquivo:
